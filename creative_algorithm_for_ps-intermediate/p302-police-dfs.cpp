...
void func(int level, int p1x_, int p1y_, int p2x_, int p2y_, int _dist) {
    if (_dist >= mini) {
        return;
    }
    if (level > N) {
    ...
    func(level + 1, P[level].first, P[level].second, p2x_, p2y_, _dist + p1_dist);
    func(level + 1, p1x_, p1y_, P[level].first, P[level].second, _dist + p2_dist);
}
